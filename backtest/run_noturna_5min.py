...
if __name__ == '__main__':
    ...
    bars1m = attach5([dict(b) for b in bars1])   # copia c/ bar5 anexado (modo misto)
    print(f"{len(bars1):,} barras 1min -> {len(bars5):,} barras 5min | canal>=40pt, pular dom, SL12.5/TP60 (j2)\n")

    print("=" * 94)
    print("  CENARIO                      taxa  aprov   d.med trades t/dia  WR    PF   PnL$/ano")
    print("=" * 94)
    print("  --- A) NOTURNA sozinha ---")
    rA1 = bt(bars1, False, True, dom);            linha("noturna 1min", rA1)
    rA5 = bt(bars5, False, True, dom);            linha("noturna 5min", rA5)
    print("  --- B) COMBINADO inteiro no mesmo TF ---")
    rB1 = bt(bars1, True, True, dom);             linha("combinado 1min (atual)", rB1)
    rB5 = bt(bars5, True, True, dom);             linha("combinado 5min", rB5)
    print("  --- C) MISTO: diurna 1min + noturna 5min ---")
    rC  = bt(bars1m, True, True, dom, mixed5=True); linha("diurna1m + noturna5m", rC)
    print("=" * 94)

    print("\n  Quebra dos trades NOTURNOS (n / t-noite-dia / WR / PF / PnL):")
    for lbl, r in [("noturna 1min", rA1), ("noturna 5min", rA5),
                   ("combinado 1min", rB1['not'] and rB1), ("combinado 5min", rB5),
                   ("misto (noturna 5min)", rC)]:
        nn = r['not']
        print(f"    {lbl:>24s}: {nn['n']:>5} trades | WR {nn['wr']:>3.0f}% | PF {nn['pf']:>4.2f} | ${nn['net']:>9,.0f}")

    print("\n" + "=" * 94)
    print("  ROBUSTEZ OUT-OF-SAMPLE — COMBINADO (1a metade | 2a metade)")
    print("=" * 94)
    meio1 = bars1[len(bars1)//2]['dt']
    def split(bs): return [b for b in bs if b['dt'] < meio1], [b for b in bs if b['dt'] >= meio1]
    b1a, b1b = split(bars1); b5a, b5b = split(bars5); bma, bmb = split(bars1m)
    for lbl, (xa, xb), mix in [("combinado 1min", (b1a, b1b), False),
                               ("combinado 5min", (b5a, b5b), False),
                               ("misto d1m+n5m", (bma, bmb), True)]:
        r1 = bt(xa, True, True, dom, mixed5=mix); r2 = bt(xb, True, True, dom, mixed5=mix)
        print(f"  {lbl:>26s}  {r1['aprov']:>2}/{r1['tot']:<2} ({r1['taxa']:>3.0f}%) PF {r1['pf']:>4.2f} "
