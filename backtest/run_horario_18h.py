...
# -*- coding: utf-8 -*-
"""
TESTE DE JANELA DE ENTRADA (pedido 14/06): e se o bot pudesse entrar ate as 18h
em vez de parar as 15h? Mantem TODA a config atual em uso (5 MNQ, tol 20 ticks,
TP 60, SL 12,5, BE +3,75->+2,5, trail 1,75, stop diario $750, sem limite de trades).

So muda a JANELA DE ENTRADA (e o flatten proporcional, +55min como o baseline).
Niveis pd_hi/pd_lo continuam sendo a max/min do RTH (9h30-16h) do dia anterior.

OBS importante: dados sao sessao eletronica quase 24h. Break do CME e 17h-18h ET
(barras das 17h quase nao existem). Entradas 16h->18h pegam fim de RTH + inicio
da sessao noturna (liquidez menor, spread maior ao vivo — atencao!).
"""
import glob, os
...
# Config atual em uso (fixa)
TOL_TICKS = 20; TP = 60.0; TRAIL = 1.75; STOP_DIA_PT = 75.0; MAX_TRD = 99
...
def bt(bars, entrada_fim_min, flatten_min, tol_ticks=TOL_TICKS, pts_tp=TP,
       pts_trail=TRAIL, pts_stop_dia=STOP_DIA_PT, max_trades=MAX_TRD):
    ...
    n_noturno = 0   # quantas entradas aconteceram depois das 16h (sessao noturna)
    ...
    for b in bars:
        ...
        if 9*60+30 <= m < 16*60:          # niveis = RTH (nao muda)
        ...
        if m >= flatten_min:
            if pos != 0: fecha(b['c'])
            continue
        if not block and pos == 0 and trades_dia < max_trades and 9*60+30 <= m < entrada_fim_min and pd_hi is not None:
    ...
    return {'n': n, 'wr': 100*len(wins)/n if n else 0, 'pf': (gw/gl if gl > 0 else 99),
            ...
            'trd_dia': n/220, 'n_noturno': n_noturno}


# (label, entrada_fim, flatten)
CENARIOS = [
    ("BASELINE 15h", 15*60,    15*60+55),
    ("ate 16h",      16*60,    16*60+55),
    ("ate 17h",      17*60,    17*60+55),
    ("ate 18h",      18*60,    18*60+55),
]
...
if __name__ == '__main__':
    ...
    print(f"{len(bars):,} barras | 5 MNQ, tol {TOL_TICKS}t, TP {TP:.0f}, trail {TRAIL}, "
          f"stop diario ${STOP_DIA_PT*MNQ_PV*N_CONTR:.0f}, sem limite\n")

    print("="*108)
    print("  EFEITO DE ESTENDER A JANELA DE ENTRADA (resto da config atual fixo)")
    print("="*108)
    print(f"  {'janela':>14s} {'taxa':>5s} {'aprov':>6s} {'reprov':>7s} {'d.mediana':>9s} "
          f"{'d.media':>7s} {'trades':>7s} {'trd/dia':>7s} {'noturnos':>8s} {'WR':>4s} {'PF':>5s} {'PnL$/ano':>10s}")
    print("-"*108)
    for label, efim, fl in CENARIOS:
        r = bt(bars, efim, fl)
        marca = "  <- ATUAL" if "BASELINE" in label else ""
        print(f"  {label:>14s} {r['taxa']:>4.0f}% {r['aprov']:>6} {r['reprov']:>7} "
              f"{r['dmediana']:>8.0f}d {r['dmed']:>6.0f}d {r['n']:>7} {r['trd_dia']:>6.1f} "
              f"{r['n_noturno']:>8} {r['wr']:>3.0f}% {r['pf']:>5.2f} {r['net']:>10,.0f}{marca}")
    print("-"*108)

    print("\n" + "="*108)
    print("  ROBUSTEZ OUT-OF-SAMPLE (1a metade x 2a metade)")
    print("="*108)
    print(f"  {'janela':>14s} {'1a metade':>26s} {'2a metade':>26s}")
    print("-"*108)
    for label, efim, fl in CENARIOS:
        r1 = bt(b1, efim, fl); r2 = bt(b2, efim, fl)
        print(f"  {label:>14s} "
