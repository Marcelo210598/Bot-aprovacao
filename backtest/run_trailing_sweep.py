...
# -*- coding: utf-8 -*-
"""
VARREDURA: TrailingPontos x BreakevenTrigPontos (18/06)
Pergunta do Marcelo: os trades estao saindo pequenos porque o trailing trava cedo
e apertado. Vale afrouxar pra deixar o trade correr? Mede com numero, sem achismo.

Engine = identica a run_segunda_domingo.py (config de producao, modo DOM-NOITE):
  5 MNQ, tol 20t, chase 15pt, stop 12.5pt, alvo 60pt, stop/dia $750, conta 25K Intraday,
  segunda usa range do domingo a noite (SegUsaDomingo=true).
So mexe em PTS_BE_TRIG e PTS_TRAIL. BE_LOCK acompanha (nunca > BE_TRIG).

Avalia: taxa de aprovacao, dias-mediana p/ aprovar, WR, PF, PnL total no periodo,
e robustez OOS (1a metade x 2a metade) pra detectar overfit.
"""
import glob, os
...
if __name__ == '__main__':
    ...
    BE_GRID = [3.75, 5, 7, 10, 15]
    TRAIL_GRID = [1.75, 3, 5, 8, 12]

    print("=" * 96)
    print("  VARREDURA TRAILING x BREAKEVEN  (* = baseline de producao)")
    print("=" * 96)
    ...
    for be in BE_GRID:
        for tr in TRAIL_GRID:
            r = bt(bars, be, tr, dom)
            tag = " *" if (be == BASE_BE_TRIG and tr == BASE_TRAIL) else ""
            label = f"BE{be:g} / tr{tr:g}{tag}"
            linha(label, r)
            resultados.append((be, tr, r))
        print()

    # melhores por taxa de aprovacao, depois por dias-mediana (menor), depois PnL
    ranked = sorted(resultados, key=lambda x: (-x[2]['taxa'], x[2]['dmediana'], -x[2]['net']))
    print("=" * 96)
    print("  TOP 5 (por taxa de aprovacao -> menos dias -> mais PnL)")
    print("=" * 96)
    for be, tr, r in ranked[:5]:
        linha(f"BE{be:g} / tr{tr:g}", r)

    print("\n" + "=" * 96)
    print("  ROBUSTEZ OUT-OF-SAMPLE: baseline + top 3 candidatos (1a metade | 2a metade)")
    print("=" * 96)
    cand = [(BASE_BE_TRIG, BASE_TRAIL)] + [(be, tr) for be, tr, _ in ranked[:3]
                                           if (be, tr) != (BASE_BE_TRIG, BASE_TRAIL)]
    for be, tr in cand:
        r1 = bt(b1, be, tr, dom); r2 = bt(b2, be, tr, dom)
        base = " (baseline)" if (be, tr) == (BASE_BE_TRIG, BASE_TRAIL) else ""
        print(f"  BE{be:g}/tr{tr:g}{base:>11s}  "
