...
# -*- coding: utf-8 -*-
"""
VARREDURA DE STOP (18/06) — reduzir o tamanho do stop ajuda a assimetria?
Marcelo: "quando vem o stop lasca tudo". Stop hoje = 12,5pt ($125 com 5 MNQ).
Testa stops menores (e maiores) mantendo o resto da config de producao (DOM-NOITE).
Engine identica a run_segunda_domingo.py. So muda PTS_SL.
"""
import glob, os
    ...
    for b in bars:
        ...
        if not block and pos == 0 and ENTRADA_INI <= m < ENTRADA_FIM and niv_hi is not None:
            ...
            if lado != 0:
                ...
                stop = c - lado * sl; target = c + lado * TP; dias.add(d)
...
def linha(label, r):
    print(f"  {label:>20s} {r['taxa']:>4.0f}% {r['aprov']:>3}/{r['tot']:<3} {r['dmediana']:>4.0f}d "
          f"{r['wr']:>4.0f}% {r['pf']:>5.2f} {r['avg_win']:>7.0f} {r['avg_loss']:>7.0f} {r['net']:>11,.0f}")
...
if __name__ == '__main__':
    ...
    print(f"{len(bars):,} barras | varia SO o stop (resto = producao). * = atual\n")
    print("=" * 92)
    print(f"  {'stop (pt)':>20s} {'taxa':>5s} {'aprov':>7s} {'dmed':>5s} {'WR':>5s} {'PF':>5s} "
          f"{'avgWin':>7s} {'avgLos':>7s} {'PnL$':>11s}")
    print("-" * 92)
    res = []
    for sl in [7.5, 10.0, 12.5, 15.0, 20.0]:
        r = bt(bars, dom, sl)
        tag = " *" if sl == BASE_SL else ""
        linha(f"SL {sl:g}pt{tag}", r); res.append((sl, r))
    print("-" * 92)
    print("\n  ROBUSTEZ OOS (1a | 2a metade) — atual + os 2 melhores por PnL:")
    ranked = sorted(res, key=lambda x: -x[1]['net'])
    cand = [BASE_SL] + [sl for sl, _ in ranked[:2] if sl != BASE_SL]
    for sl in cand[:3]:
        r1 = bt(b1, dom, sl); r2 = bt(b2, dom, sl)
        b = " (atual)" if sl == BASE_SL else ""
        print(f"    SL {sl:g}{b:>8s}  {r1['taxa']:>3.0f}% PF {r1['pf']:.2f} ${r1['net']:>9,.0f}  |  "
