...
def bt(bars, usar_diurna=True, usar_noturna=True, dom_map=None, retouch=False, pular_dom=True):
    ...
    for b in bars:
        ...
        if usar_noturna:
            ...
            if na_janela:
                if dbr != noite_dia:
                    noite_dia = dbr; noite_hi = h; noite_lo = l; pend = None; bars_new_hi = 0; bars_new_lo = 0; pode_c = True; pode_v = True
                else:
            ...
            if pode:
                ...
                # preco SAIU da zona? (vela toda fora da linha) -> libera toque fresco
                if l > z_compra: pode_c = True
                if h < z_venda:  pode_v = True
                if pos == 0 and pend is None:
                    if h >= z_venda and (not retouch or pode_v):
                        pend = (-1, min(o, c), GAT); pode_v = False
                    elif l <= z_compra and (not retouch or pode_c):
                        pend = (1, max(o, c), GAT); pode_c = False
            elif not na_janela:
                pend = None
...
if __name__ == '__main__':
    ...
    print(f"{len(bars):,} barras | TOQUE FRESCO: so re-arma depois do preco SAIR da zona e VOLTAR a tocar\n")
    print("=" * 100)
    print("  versao            taxa  aprov  d.med | NOITE: trades  WR   PF    PnL$ | COMBINADO/ano | OOS(1a|2a)")
    print("=" * 100)
    for lbl, rt in [("BASELINE (atual)", False), ("TOQUE FRESCO", True)]:
        rc = bt(bars, True, True, dom, retouch=rt)
        r1 = bt(b1, True, True, dom, retouch=rt); r2 = bt(b2, True, True, dom, retouch=rt)
        nn = rc['not']
        print(f"  {lbl:>16s}  {rc['taxa']:>4.0f}% {rc['aprov']:>3}/{rc['tot']:<3} {rc['dmediana']:>4.0f}d | "
              f"{nn['n']:>6} {nn['wr']:>4.0f}% {nn['pf']:>4.2f} {nn['net']:>8,.0f} | "
              f"{rc['net']:>10,.0f}     | {r1['taxa']:>3.0f}%|{r2['taxa']:>3.0f}%")
    print("=" * 100)
