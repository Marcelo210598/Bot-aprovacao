...
def bt(bars, usar_diurna=True, usar_noturna=True, dom_map=None, estavel=0, pular_dom=True):
        ...
        if usar_noturna:
            ...
            if pode:
                ...
                if pos == 0 and pend is None:
                    # FILTRO ANTI-TENDENCIA: so fada se o extremo esta estavel ha >= 'estavel' barras
                    if h >= z_venda and bars_new_hi >= estavel:
                        pend = (-1, min(o, c), GAT)
                    elif l <= z_compra and bars_new_lo >= estavel:
                        pend = (1, max(o, c), GAT)
            elif not na_janela:
                pend = None
...
if __name__ == '__main__':
    ...
    print(f"{len(bars):,} barras | filtro: so fada se o extremo do canal estiver estavel ha >=N barras\n")
    print("=" * 100)
    print("  N (estab.)   taxa  aprov  d.med | NOITE: trades  WR   PF    PnL$ | COMBINADO PnL$/ano | OOS(1a|2a)")
    print("=" * 100)
    for N in [0, 1, 2, 3, 5]:
        rc = bt(bars, True, True, dom, estavel=N)
        r1 = bt(b1, True, True, dom, estavel=N); r2 = bt(b2, True, True, dom, estavel=N)
        nn = rc['not']
        tag = "  <- baseline (sem filtro)" if N == 0 else ""
        print(f"   N={N:<2}        {rc['taxa']:>4.0f}% {rc['aprov']:>3}/{rc['tot']:<3} {rc['dmediana']:>4.0f}d | "
              f"{nn['n']:>6} {nn['wr']:>4.0f}% {nn['pf']:>4.2f} {nn['net']:>8,.0f} | "
              f"{rc['net']:>10,.0f}        | {r1['taxa']:>3.0f}%|{r2['taxa']:>3.0f}%{tag}")
    print("=" * 100)
    print("  (NOITE = so a parte noturna; COMBINADO = conta inteira diurna+noturna)")
