...
def bt(bars, usar_diurna=True, usar_noturna=True, dom_map=None, tol_pts=5.0, pular_dom=True):
    ...
    for b in bars:
        ...
        if usar_noturna:
            ...
            if pode:
                ...
                z_venda = noite_hi - tol_pts; z_compra = noite_lo + tol_pts
                if pend is not None:
...
if __name__ == '__main__':
    ...
    dom = domingo_ranges(bars); meio = bars[len(bars)//2]['dt']
    b1=[b for b in bars if b['dt']<meio]; b2=[b for b in bars if b['dt']>=meio]
    print(f"{len(bars):,} barras | GATILHO = tocar a LINHA (extremo 0%/100%) dentro de X pontos\n")
    print("="*98)
    print("  tol (linha)   taxa  aprov  d.med | NOITE: trades  WR   PF    PnL$ | COMBINADO/ano | OOS")
    print("="*98)
    # baseline Fib 23.6% pra referencia
    import importlib.util
    spec=importlib.util.spec_from_file_location("z","backtest/run_noturna_zona.py"); zmod=importlib.util.module_from_spec(spec); spec.loader.exec_module(zmod)
    rb=zmod.bt(bars,True,True,dom,fib_c=0.236); r1b=zmod.bt(b1,True,True,dom,fib_c=0.236); r2b=zmod.bt(b2,True,True,dom,fib_c=0.236)
    nnb=rb['not']
    print(f"  Fib 23,6% (atual) {rb['taxa']:>4.0f}% {rb['aprov']:>3}/{rb['tot']:<3} {rb['dmediana']:>4.0f}d | {nnb['n']:>6} {nnb['wr']:>4.0f}% {nnb['pf']:>4.2f} {nnb['net']:>8,.0f} | {rb['net']:>10,.0f}  | {r1b['taxa']:.0f}/{r2b['taxa']:.0f}")
    print("  "+"-"*96)
    for tp in [2.0,5.0,10.0,15.0]:
        rc=bt(bars,True,True,dom,tol_pts=tp); r1=bt(b1,True,True,dom,tol_pts=tp); r2=bt(b2,True,True,dom,tol_pts=tp)
        nn=rc['not']
        print(f"  linha +/-{tp:>4.1f}pt {rc['taxa']:>4.0f}% {rc['aprov']:>3}/{rc['tot']:<3} {rc['dmediana']:>4.0f}d | {nn['n']:>6} {nn['wr']:>4.0f}% {nn['pf']:>4.2f} {nn['net']:>8,.0f} | {rc['net']:>10,.0f}  | {r1['taxa']:.0f}/{r2['taxa']:.0f}")
    print("="*98)
