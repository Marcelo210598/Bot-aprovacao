...
// =============================================================================
//  BotAprovacao_BASELINE_REPLAY — copia EXATA do baseline (mesma logica, byte a
//  byte identica em toda a gestao de entrada/saida), so renomeada pra poder
//  rodar LADO A LADO com a versao experimental no mesmo Market Replay, no mesmo
//  grafico/instrumento, sem colidir com a classe "BotAprovacao" de producao.
//
//  Criada em 20/08/2026 a pedido do Marcelo, pra permitir o A/B real em Replay:
//  BASELINE_REPLAY vs SaidaParcial_REPLAY, mesmos dados, mesmo timeframe, ao
//  mesmo tempo. NAO ALTERAR NENHUMA LINHA DE LOGICA — so o necessario pra
//  coexistir (nome da classe, Name, arquivo de PnL diario isolado — ver nota
//  abaixo). Fonte: BotAprovacao_BASELINE_BACKUP.cs (confirmado 20/08 como
//  byte-identico a BotAprovacao.cs de producao antes desta copia).
//
//  ⚠️ ARQUIVO DE PNL DIARIO ISOLADO: o baseline original grava o stop diario em
//  "BotAprovacao_pnl_diario.txt" — o MESMO arquivo que a producao usa. Se essa
//  classe rodasse com esse nome ao lado da producao (ou da SaidaParcial_REPLAY,
//  que tambem precisou do mesmo ajuste), as duas ficariam lendo/escrevendo o
//  stop diario uma da outra, invalidando a comparacao. Aqui o caminho foi
//  trocado para "BotAprovacao_BASELINE_REPLAY_pnl_diario.txt" (ver
//  CaminhoArquivoPnl() mais abaixo) — unica mudanca de comportamento deste
//  arquivo, e e' puramente de infraestrutura (isolamento de estado), nao mexe
//  em nenhuma regra de entrada/saida.
// =============================================================================
...
{
	public class BotAprovacao_BASELINE_REPLAY : Strategy
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description					= @"BASELINE_REPLAY — copia exata do BotAprovacao original, so renomeada p/ rodar lado a lado com a SaidaParcial_REPLAY no mesmo Market Replay (A/B controlado). Logica 100% identica a producao.";
				Name						= "BotAprovacao_BASELINE_REPLAY";
		...
		{
			// Isolado da producao e da SaidaParcial_REPLAY (ver nota no topo do arquivo) —
			// evita que duas estrategias rodando juntas no Replay leiam/escrevam o mesmo
			// stop diario uma da outra.
			return Path.Combine(
				Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
				"NinjaTrader 8", "BotAprovacao_BASELINE_REPLAY_pnl_diario.txt");
		}
