...
// =============================================================================
//  BotAprovacao_SaidaParcial_REPLAY_DEBUG — mesma logica EXATA da
//  SaidaParcial_REPLAY (nenhuma condicao de entrada/saida foi alterada), com
//  LOGGING ADICIONAL pra reconstruir o que aconteceu tick a tick num trade
//  especifico durante o Market Replay. Criada 20/08/2026.
//
//  O QUE FOI ADICIONADO (só Print()/estado de leitura, zero decisao nova):
//    - TRADE OPEN: timestamp, direcao, qtd, entrada, SL inicial, ID do trade
//      (= sinalAtivo, ex. "NIV_S7" — use esse ID pra filtrar o log de 1 trade).
//    - MAE (maximo adverso), rastreado tick a tick junto com o MFE (favPrice)
//      que ja existia. So leitura/registro — nao entra em nenhuma decisao de
//      stop/alvo/parcial.
//    - BE ATIVADO: log dedicado no instante exato em que beFeito vira true
//      (o codigo original nunca logava esse momento, so o resultado final).
//    - PARCIAL: log enriquecido com qtd antes/enviada/restante e preco-alvo
//      vs preco de mercado no tick do gatilho.
//    - HEARTBEAT "DURANTE O TRADE": 1 linha por barra nova (nao por tick, pra
//      nao inundar o log) com estado atual (fav/MAE/distancia ate a parcial/
//      BE feito?/parcial feita?/stop atual). Tick a tick completo e' opcional
//      via [EXP-DEBUG] LogTickPorTick=true (log MUITO grande, so ligar pra
//      auditar 1 dia especifico).
//    - EXECUCAO REAL: hook em OnExecutionUpdate pra logar o preco de fill
//      DE VERDADE (nao so o preco do tick que disparou a ordem) da saida
//      parcial e da saida final, com slippage = fill - gatilho.
//    - SAIDA final: mesmo log de sempre + MAE (que nao existia).
//
//  IMPORTANTE (limite conhecido, ver auditoria Fase 1 de 20/08): esses logs
//  registram a sequencia real de ticks DENTRO do Market Replay — isso e'
//  informacao real, nao inventada, porque o Replay entrega tick a tick de
//  verdade (State.Realtime). O que esses logs NAO resolvem e' a comparacao
//  contra o backtest de barra de 1min (Python/Strategy Analyzer), que so tem
//  High/Low agregado da barra e não sabe a ordem intrabar — ao comparar os
//  dois, qualquer caso em que o backtest precisar "adivinhar" a ordem dentro
//  de uma barra deve ser marcado como AMBIGUO no relatorio, nunca assumido.
// =============================================================================
...
{
	public class BotAprovacao_SaidaParcial_REPLAY_DEBUG : Strategy
	{
		...
		// ---------- [DEBUG] auditoria trade a trade (so leitura/log, nao entra em nenhuma decisao) ----------
		private double maePrice           = 0;     // pior excursao (MAE) — espelha o favPrice, mas pro lado ruim
		private double precoTriggerParcial = 0;    // preco de mercado no tick que MANDOU a ordem da parcial (p/ slippage no fill real)
		private double precoTriggerFinal   = 0;    // idem, no tick que mandou a saida final (stop/trailing/alvo)
		private string ultimoMotivoSaida   = "";   // guarda o motivo da saida final p/ o log de execucao (OnExecutionUpdate)
		private string lastBarKeyLogado    = "";   // throttle do heartbeat: 1 linha por barra nova, nao por tick
		...
		protected override void OnStateChange()
		{
			...
			{
				Description					= @"SaidaParcial_REPLAY_DEBUG — mesma logica exata da SaidaParcial_REPLAY, com logging detalhado (TRADE OPEN, BE ativado, PARCIAL, execucao real, MFE/MAE, heartbeat por barra) p/ auditar trade a trade no Market Replay. Zero mudanca de decisao.";
				Name						= "BotAprovacao_SaidaParcial_REPLAY_DEBUG";
				...
				// ----- EXPERIMENTO 18/08: saida parcial E(4+1) -----
				// [DECISAO 20/08] Desligada por padrao — ver docs/taxa-aprovacao-30dias-20-08.md
				// e docs/melhorias-sugeridas.md item #15 (piora consistente em todos os testes).
				...
				// ----- [DEBUG] auditoria -----
				LogTickPorTick		= false;  // OFF por padrao (log gigante). Ligar so pra auditar 1 dia especifico.
		...
		protected override void OnMarketData(MarketDataEventArgs e)
		{
			...
			if (!gerenciando)
			{
				...
				favPrice   = entryPrice;
				maePrice   = entryPrice;   // [DEBUG] MAE comeca zerada, so anda pro lado adverso
				beFeito    = false;
				...
				parcialFeita  = false;
				pnlParcialUsd = 0;
				lastBarKeyLogado    = "";   // [DEBUG] reseta o throttle do heartbeat pro trade novo
				precoTriggerParcial = 0;    // [DEBUG] reseta os campos de correlacao c/ OnExecutionUpdate
				precoTriggerFinal   = 0;
				ultimoMotivoSaida   = "";

				// [DEBUG] TRADE OPEN — 1 log por trade, campos pedidos: timestamp, instrumento, direcao,
				// qtd, entrada, SL inicial, ID do trade. ID = sinalAtivo (ex. "NIV_S7") -- use esse texto
				// pra filtrar TODO o log de 1 trade especifico (Print search no NinjaScript Output).
				Print(string.Format("{0}  [DEBUG] ===== TRADE OPEN ===== id={1} | {2} | {3} | qtd={4} | entrada={5:F2} | SLinicial={6:F2} ({7:F2}pt)",
					Time[0], sinalAtivo, Instrument.FullName, isLong ? "LONG" : "SHORT", Contratos, entryPrice, stopPrice, StopPontos));
			...
			if (UsarSaidaParcial && !parcialFeita && Position.Quantity > ParcialContratos)
			{
				...
				{
					int qtdAntes = Position.Quantity;   // [DEBUG] quantidade ANTES da ordem sair
					parcialFeita = true;
					precoTriggerParcial = preco;   // [DEBUG] preco do TICK que disparou -- comparar com fill real em OnExecutionUpdate
					...
					Print(string.Format("{0}  <<< SAIDA PARCIAL [{1}] | id={2} | {3} | preco mercado no gatilho={4:F2} | "
						+ "preco-alvo={5:F2} | qtd antes={6} | qtd enviada p/ saida={7} | qtd restante esperada={8} | "
						+ "PnL parcela ~${9:F2}  [DEBUG: preco de FILL real vem no log EXECUCAO REAL logo abaixo]",
						Time[0], sinalParcial, sinalAtivo, isLong ? "LONG" : "SHORT", preco, alvoParcialPrice,
						qtdAntes, ParcialContratos, qtdAntes - ParcialContratos, pnlParcialUsd));
				}
			}
			...
			if (isLong)
			{
				...
				maePrice = Math.Min(maePrice, preco);   // [DEBUG] pior preco contra (MAE), so leitura
				if (!beFeito && (favPrice - entryPrice) >= BreakevenTrigPontos)
				{
					...
					// [DEBUG] QUANDO BE FOR ATIVADO: timestamp, entrada, preco atual, MFE, calculo do lock, novo stop.
					Print(string.Format("{0}  [DEBUG] ===== BE ATIVADO ===== id={1} | entrada={2:F2} | precoAtual={3:F2} | "
						+ "MFE={4:F2}pt | modo={5} | lock={6:F2}pt (=> {7:F2}) | novoStop={8:F2}",
						Time[0], sinalAtivo, entryPrice, preco, favPrice - entryPrice,
						UsarBELockProporcional ? string.Format("proporcional({0:F2}xMFE)", BELockFracaoMFE) : "fixo",
						lockPts0, entryPrice + lockPts0, stopPrice));
				}
				if (beFeito)
				{
			...
			else
			{
				...
				maePrice = Math.Max(maePrice, preco);   // [DEBUG] pior preco contra (MAE), so leitura
				if (!beFeito && (entryPrice - favPrice) >= BreakevenTrigPontos)
				{
					...
					Print(string.Format("{0}  [DEBUG] ===== BE ATIVADO ===== id={1} | entrada={2:F2} | precoAtual={3:F2} | "
						+ "MFE={4:F2}pt | modo={5} | lock={6:F2}pt (=> {7:F2}) | novoStop={8:F2}",
						Time[0], sinalAtivo, entryPrice, preco, entryPrice - favPrice,
						UsarBELockProporcional ? string.Format("proporcional({0:F2}xMFE)", BELockFracaoMFE) : "fixo",
						lockPts0, entryPrice - lockPts0, stopPrice));
				}
				if (beFeito)
			...
			// [DEBUG] tick a tick opcional (log grande, so liga LogTickPorTick pra auditar 1 dia especifico)
			if (LogTickPorTick)
			{
				double mfeAtual = isLong ? (favPrice - entryPrice) : (entryPrice - favPrice);
				double maeAtual = isLong ? (entryPrice - maePrice) : (maePrice - entryPrice);
				double distParcial = UsarSaidaParcial && !parcialFeita
					? (isLong ? (entryPrice + ParcialAlvoPontos - preco) : (preco - (entryPrice - ParcialAlvoPontos)))
					: 0;
				Print(string.Format("{0}  [DEBUG][TICK] id={1} | preco={2:F2} | MFE={3:F2}pt | MAE={4:F2}pt | "
					+ "distAteParcial={5:F2}pt | BE={6} | parcialFeita={7} | stopAtual={8:F2}",
					Time[0], sinalAtivo, preco, mfeAtual, maeAtual, distParcial,
					beFeito ? "sim" : "nao", parcialFeita ? "sim" : "nao", stopPrice));
			}

			// [DEBUG] heartbeat: 1 linha por barra NOVA (nao por tick) enquanto em posicao — "DURANTE O TRADE"
			string barKeyAtual = Time[0].ToString("yyyy-MM-dd HH:mm");
			if (barKeyAtual != lastBarKeyLogado)
			{
				lastBarKeyLogado = barKeyAtual;
				double mfeAtual = isLong ? (favPrice - entryPrice) : (entryPrice - favPrice);
				double maeAtual = isLong ? (entryPrice - maePrice) : (maePrice - entryPrice);
				double distParcial = UsarSaidaParcial && !parcialFeita
					? (isLong ? (entryPrice + ParcialAlvoPontos - preco) : (preco - (entryPrice - ParcialAlvoPontos)))
					: 0;
				Print(string.Format("{0}  [DEBUG] --- durante o trade --- id={1} | preco={2:F2} | MFE={3:F2}pt | MAE={4:F2}pt | "
					+ "distAteParcial={5:F2}pt | BE={6} | parcialFeita={7} | stopAtual={8:F2}",
					Time[0], sinalAtivo, preco, mfeAtual, maeAtual, distParcial,
					beFeito ? "sim" : "nao", parcialFeita ? "sim" : "nao", stopPrice));
			}
			...
			if ((isLong && preco >= alvoPrice) || (!isLong && preco <= alvoPrice))
			{
				...
				precoTriggerFinal = preco;     // [DEBUG] p/ slippage no fill real (OnExecutionUpdate)
				ultimoMotivoSaida = "AlvoTick";
			...
			if ((!isLong && preco >= stopPrice) || (isLong && preco <= stopPrice))
			{
				stopIntrabarEnviado = true;
				precoTriggerFinal = preco;     // [DEBUG] p/ slippage no fill real (OnExecutionUpdate)
				ultimoMotivoSaida = beFeito ? "TrailingTick" : "StopTick";
		...
		// [DEBUG] EXECUCAO REAL — hook em cima do preenchimento de ordens do NinjaTrader/broker.
		// So LEITURA/LOG: nao manda ordem nenhuma, nao muda decisao nenhuma. Objetivo: comparar o
		// preco do TICK que disparou a ordem (guardado em precoTriggerParcial/precoTriggerFinal, ver
		// acima) com o preco de FILL REAL que o servidor devolve -- essa diferenca e' o slippage de
		// verdade, coisa que o backtest de barra nao tem como enxergar.
		protected override void OnExecutionUpdate(Execution execution, string executionId, double price, int quantity,
			MarketPosition marketPosition, string orderId, DateTime time)
		{
			if (execution?.Order == null) return;
			string nomeOrdem = execution.Order.Name ?? "";

			if (nomeOrdem.StartsWith("X_Parcial_"))
			{
				double slippage = precoTriggerParcial == 0 ? 0 : (price - precoTriggerParcial);
				Print(string.Format("{0}  [DEBUG] ===== EXECUCAO REAL (PARCIAL) ===== id={1} | ordem={2} | "
					+ "qtd preenchida={3} | preco de FILL={4:F2} | preco do gatilho (tick)={5:F2} | slippage={6:F2}pt "
					+ "({7})",
					time, sinalAtivo, nomeOrdem, quantity, price, precoTriggerParcial, slippage,
					slippage == 0 ? "sem diferenca" : (Math.Abs(slippage) < 0.01 ? "desprezivel" : "REAL — ver sinal p/ saber se foi a favor ou contra")));
			}
			else if (nomeOrdem.StartsWith("X_"))
			{
				double slippage = precoTriggerFinal == 0 ? 0 : (price - precoTriggerFinal);
				Print(string.Format("{0}  [DEBUG] ===== EXECUCAO REAL (SAIDA FINAL, motivo={1}) ===== id={2} | ordem={3} | "
					+ "qtd preenchida={4} | preco de FILL={5:F2} | preco do gatilho (tick)={6:F2} | slippage={7:F2}pt "
					+ "({8})",
					time, ultimoMotivoSaida, sinalAtivo, nomeOrdem, quantity, price, precoTriggerFinal, slippage,
					slippage == 0 ? "sem diferenca" : (Math.Abs(slippage) < 0.01 ? "desprezivel" : "REAL — ver sinal p/ saber se foi a favor ou contra")));
			}
		}
		...
		private void GerenciaPosicao()
		{
			...
			if (!gerenciando)
			{
				entryPrice = Position.AveragePrice;
				favPrice   = entryPrice;
				maePrice   = entryPrice;   // [DEBUG]
				beFeito    = false;
				stopPrice  = isLong ? entryPrice - StopPontos : entryPrice + StopPontos;
				alvoPrice  = isLong ? entryPrice + AlvoPontos : entryPrice - AlvoPontos;
				gerenciando = true;
				// [DEBUG] este caminho (por barra) so deveria inicializar um trade se, por algum
				// motivo, OnMarketData nao rodou primeiro (em Market Replay isso e' raro — OnMarketData
				// roda em TODO tick). Se este log aparecer, vale investigar por que o caminho tick nao
				// pegou o fill antes. IMPORTANTE: este caminho NAO tem a logica de saida parcial
				// (so existe em OnMarketData, ver auditoria Fase 1) — se um trade for gerido
				// inteiramente por aqui, a parcial de 20pt simplesmente nao vai disparar.
				Print(string.Format("{0}  [DEBUG] ===== TRADE OPEN (via GerenciaPosicao/barra, NAO via tick!) ===== "
					+ "id={1} | {2} | qtd={3} | entrada={4:F2} | SLinicial={5:F2}",
					Time[0], sinalAtivo, isLong ? "LONG" : "SHORT", Contratos, entryPrice, stopPrice));
			}
			...
			if (isLong)
			{
				...
				maePrice = Math.Min(maePrice, Low[0]);   // [DEBUG]
				if (!beFeito && (favPrice - entryPrice) >= BreakevenTrigPontos)
				{
					beFeito = true;
					// EXPERIMENTO 19/08: lock proporcional ao MFE ja alcancado, em vez de fixo.
					double lockPts0 = UsarBELockProporcional ? (favPrice - entryPrice) * BELockFracaoMFE : BreakevenLockPontos;
					Print(string.Format("{0}  [DEBUG] ===== BE ATIVADO (via barra) ===== id={1} | entrada={2:F2} | High[0]={3:F2} | "
						+ "MFE={4:F2}pt | lock={5:F2}pt", Time[0], sinalAtivo, entryPrice, High[0], favPrice - entryPrice, lockPts0));
			...
			else
			{
				...
				maePrice = Math.Max(maePrice, High[0]);   // [DEBUG]
				if (!beFeito && (entryPrice - favPrice) >= BreakevenTrigPontos)
				{
					beFeito = true;
					double lockPts0 = UsarBELockProporcional ? (entryPrice - favPrice) * BELockFracaoMFE : BreakevenLockPontos;
					Print(string.Format("{0}  [DEBUG] ===== BE ATIVADO (via barra) ===== id={1} | entrada={2:F2} | Low[0]={3:F2} | "
						+ "MFE={4:F2}pt | lock={5:F2}pt", Time[0], sinalAtivo, entryPrice, Low[0], entryPrice - favPrice, lockPts0));
					// Move stop servidor para nivel de breakeven — garante lucro minimo intrabar.
					if (!string.IsNullOrEmpty(sinalAtivo) && sinalAtivo != "RECOVERY")
						SetStopLoss(sinalAtivo, CalculationMode.Price, entryPrice - lockPts0 + BufferStopServidorPontos, false);
					// em recovery: server stop anterior ainda protege; trailing sintetico gerencia a saida
		...
		private void FechaPosicao(string motivo)
		{
			...
			// [DEBUG] rede de seguranca: garante que ultimoMotivoSaida/precoTriggerFinal estao populados
			// mesmo pros motivos que nao passam pelo bloco intrabar (FlattenEOD, StopDiario, MetaAtingida,
			// ou o caminho de barra em GerenciaPosicao) -- assim OnExecutionUpdate sempre tem o que comparar.
			ultimoMotivoSaida = motivo;
			if (precoTriggerFinal == 0) precoTriggerFinal = saidaAprox;
			bool isLongMae = Position.MarketPosition == MarketPosition.Long;
			double maeFinal = isLongMae ? (entryPrice - maePrice) : (maePrice - entryPrice);
			Print(string.Format("{0}  <<< SAIDA [{1}] | id={2} | {3} | entrada {4:F2} ~saida {5:F2} | MFE(fav) {6:F2} | MAE {7:F2}pt | BE={8}",
				Time[0], motivo, sinalAtivo, Position.MarketPosition, entryPrice, saidaAprox, favPrice, maeFinal, beFeito ? "sim" : "nao"));
		...
		{
			// Isolado da producao e da BASELINE_REPLAY (ver nota no topo do arquivo) —
			// evita que duas estrategias rodando juntas no Replay leiam/escrevam o mesmo
			// stop diario uma da outra.
			return Path.Combine(
				Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
				"NinjaTrader 8", "BotAprovacao_SaidaParcial_REPLAY_DEBUG_pnl_diario.txt");
		}
		...
		// ----- [DEBUG] auditoria trade a trade -----
		[NinjaScriptProperty]
		[Display(Name="[DEBUG] Log tick a tick", Description="OFF (padrao) = so os eventos importantes (open/BE/parcial/execucao/saida) + 1 linha por barra. ON = 1 linha POR TICK enquanto em posicao -- log MUITO grande, so ligar pra auditar 1 dia especifico de cada vez.", Order=100, GroupName="11. Debug/Auditoria")]
		public bool LogTickPorTick { get; set; }
