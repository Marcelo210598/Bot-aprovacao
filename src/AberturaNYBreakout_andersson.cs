...
{
    public class AberturaNYBreakout : Strategy
    {
        // ---- Estado interno ----
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (agora.Date != diaAtual)
            {
                ...
                if (ModoDebug)
                    Print(string.Format("{0} | Novo dia. Janela de entrada: {1} até {2}", agora, horaInicioJanela, horaFimJanela));
            }
            ...
            if (agora >= horaFimJanela)
            {
                if (monitorando && ModoDebug)
                    Print(string.Format("{0} | Janela de entrada encerrada sem gatilho de {1} ticks.", agora, GatilhoEntradaTicks));
                monitorando = false;
                return;
            }
            ...
            if (!monitorando)
            {
                ...
                if (ModoDebug)
                    Print(string.Format("{0} | Referência de abertura capturada: {1}", agora, referenciaAbertura));
            }
            ...
            if (movimentoAtual >= gatilhoPontos)
            {
                ...
                if (ModoDebug)
                    Print(string.Format("{0} | GATILHO DE COMPRA. Ref={1} Preço atual={2} Mov=${3:0.00}",
            ...
            else if (-movimentoAtual >= gatilhoPontos)
            {
                ...
                if (ModoDebug)
                    Print(string.Format("{0} | GATILHO DE VENDA. Ref={1} Preço atual={2} Mov=${3:0.00}",
                        agora, referenciaAbertura, Closes[1][0], -movimentoAtual * pointValue));
            }
        }
        ...
        private void GerenciarBreakeven(double pointValue, double tickSize)
        {
            ...
            if (!travaLucroAplicada && lucroAtualDolares >= TravaLucroGatilhoDolares)
            {
                ...
                if (ModoDebug)
                    Print(string.Format("{0} | TRAVA DE LUCRO aplicada (${1}). Novo stop = {2}", Times[1][0].AddMinutes(-1), TravaLucroGatilhoDolares, novoStop));
            ...
            if (lucroAtualDolares >= BreakevenGatilhoDolares)
            {
                ...
                if (ModoDebug)
                    Print(string.Format("{0} | BREAKEVEN aplicado. Novo stop = {1}", Times[1][0].AddMinutes(-1), novoStop));
            }
        }
            ...
            int quantity, MarketPosition marketPosition, string orderId, DateTime time)
        {
            ...
            if (execution.Order.Name == "EntradaCompra" && marketPosition == MarketPosition.Long)
            {
                ...
                if (ModoDebug)
                    Print(string.Format("{0} | Compra preenchida @ {1} | Stop={2} Alvo={3}", time, precoEntrada, stopPrice, targetPrice));
            }
            else if (execution.Order.Name == "EntradaVenda" && marketPosition == MarketPosition.Short)
            {
                ...
                if (ModoDebug)
                    Print(string.Format("{0} | Venda preenchida @ {1} | Stop={2} Alvo={3}", time, precoEntrada, stopPrice, targetPrice));
            }
            ...
            if (Position.MarketPosition == MarketPosition.Flat)
            {
                if (ModoDebug && (execution.Order.Name == "StopLoss" || execution.Order.Name == "TakeProfit"))
                    Print(string.Format("{0} | Posição encerrada por {1} @ {2}", time, execution.Order.Name, price));
