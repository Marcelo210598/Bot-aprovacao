...
// =============================================================================
//  BotAprovacaoDow_MYM — EXPERIMENTO direcao B (01/09/2026)
// -----------------------------------------------------------------------------
//  DERIVADO de src/BotAprovacao_BETrigger25.cs (o "3o bot" da linhagem
//  BotAprovacao -> SaidaParcial -> BETrigger25). MESMA logica de entrada,
//  gestao, recovery, stop servidor e horarios — SO MUDA o instrumento e os
//  parametros escalados. NAO E' O BOT DE PRODUCAO.
//
//  POR QUE ESTE BOT EXISTE (ver docs/bot-mym-dd-estatico.md + docs/melhorias-
//  sugeridas.md #22):
//
//  A varredura da "direcao B" (01/09) testou a reversao PDH/PDL em MES/M2K/MYM
//  com dado real do Databento e o motor de aprovacao de 30 dias:
//    - MES (S&P):    PF max 0,80  -> SEM EDGE (o S&P atravessa os niveis)
//    - M2K (Russell):PF max 0,85  -> SEM EDGE (small caps rompem por momentum)
//    - MYM (Dow):    PF 1,44 com fill de 1 tick / 1,18 com 2 ticks -> UNICO com
//                    edge. WR 76% (> NQ 68%), quase nao estoura (o Dow nao da
//                    spike contra). MAS: FRAGIL a slippage — so forward test ao
//                    vivo resolve se o fill real do MYM aguenta.
//
//  DUAS alavancas juntas neste bot:
//    (1) Firma de DD ESTATICO (Tradeify / MyFundedFutures / TPT) em vez da Apex.
//        A Apex usa DD trailing intradiario (conta lucro nao-realizado no pico)
//        -> e o que fabrica o risco de estouro. Com DD estatico (limite fixo do
//        saldo inicial), a estrategia NUNCA estoura -> ou aprova ou so expira.
//        Backtest NQ: 55% -> 60% e ZERO estouros so trocando o modelo de DD.
//        Este bot NAO tem guarda de DD trailing interna (nunca teve — sempre
//        confiou no StopDiarioDolar). So setar Meta/StopDiario/MinDias conforme
//        a firma escolhida (ver docs/bot-mym-dd-estatico.md).
//    (2) MYM (Micro E-mini Dow Jones, $0,50/ponto, tick = 1,00 ponto Dow).
//
//  PARAMETROS (defaults abaixo, do grid de 01/09, range RTH mediano do MYM ~417pt):
//    Contratos 5 (backtest tem pico em 8-10) | Alvo 117 | Stop 29 | BE +9->+6 |
//    trail 2 | TolToqueTicks 12 (= 12pt, tick do MYM = 1pt) | MaxDist 35.
//
//  ⚠️ NAO E' VALIDADO AO VIVO. O backtest tem edge (PF 1,18-1,44) mas depende
//  do fill real. Forward test no Market Replay ANTES de qualquer conta real,
//  registrando o preco de fill de cada trade (o log ja mostra).
// -----------------------------------------------------------------------------
//  Estrategia: reversao na maxima/minima do dia anterior ("Niveis 94").
//  Config 5 MYM: TP 117 | SL 29 | BE +9->+6 | trail 2 | tol 12 ticks | maxDist 35pt.
//
...
//  Calculate = OnBarClose. MYM: $0,50/ponto; 5 contratos = $2,50/ponto.
...
{
	public class BotAprovacaoDow_MYM : Strategy
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description					= @"EXPERIMENTO 01/09 (direcao B) — reversao PDH/PDL no MYM (Micro E-mini Dow, $0,50/pt, tick 1pt) com params escalados p/ o Dow, p/ firma de DD ESTATICO (Tradeify/MFFU/TPT). Derivado do BotAprovacao_BETrigger25. Backtest: PF 1,18-1,44 (depende do fill). NAO VALIDADO AO VIVO — forward test no Market Replay antes de conta real. Ver docs/bot-mym-dd-estatico.md.";
				Name						= "BotAprovacaoDow_MYM";
				...
				// ===== PARAMS ESCALADOS PRO MYM (Micro Dow, tick=1pt, $0,50/pt) =====
				// Do grid de 01/09 (backtest/run_instrumento_scan.py), range RTH mediano
				// do MYM ~417pt. NQ usa SL 12,5 / TP 60 / tol 5pt / maxdist 15; aqui tudo
				// escalado pro tamanho do Dow. Ver docs/bot-mym-dd-estatico.md.
				Contratos			= 5;      // backtest tem pico em 8-10c; comeca conservador (5c x $0,50 = $2,50/pt, SL 29pt = $72,50/trade)

				AlvoPontos			= 117.0;  // ~4x o stop (grid: SL*4)
				StopPontos			= 29.0;   // ~7% do range RTH mediano do MYM
				BreakevenTrigPontos	= 9.0;    // ~SL*0,3
				BreakevenLockPontos	= 6.0;    // ~SL*0,2
				TrailingPontos		= 2.0;    // ~SL*0,07 (no NQ era 1,75; MYM tick=1pt)
				TolToqueTicks		= 12;     // MYM tick = 1pt -> 12 ticks = 12pt de tolerancia no toque da linha
				MaxDistPontos		= 35.0;   // nao entra se o close ficou > 35pt alem da linha (anti-chase)
				BufferStopServidorPontos = 10.0;  // stop servidor 10pt mais largo que o gerenciado (Dow anda mais em pontos)

				StopDiarioDolar		= 750.0;  // ⚠️ AJUSTAR conforme a firma de DD estatico (ver docs). Apex = 750.
				MaxTradesDia		= 12;   // limite anti-overtrading (mesmo do NQ)
				...
				UsarBELockProporcional	= false;  // [MYM 01/09] DESLIGADO: era uma otimizacao marginal do NQ (Delta +$122/mes),
				BELockFracaoMFE			= 0.85;   // nunca validada no Dow. Com trail 2 / BE trig 9, so valeria com fracao > 1-2/9 = 0,78.
				                                  // Deixado 0,85 pra quando/se for testar; por ora OFF = lock fixo BreakevenLockPontos.
				...
				PararAoAprovar		= true;
				MetaLucroDolar		= 1500.0;  // ⚠️ AJUSTAR conforme a firma (Apex 25K=1500; Tradeify/MFFU variam). Ver docs/bot-mym-dd-estatico.md.
				MinDiasOperados		= 7;       // ⚠️ AJUSTAR conforme a firma (Apex=7; muitas de DD estatico = 2-5).
		...
		protected override void OnMarketData(MarketDataEventArgs e)
		{
			...
			if (UsarSaidaParcial && !parcialFeita && Position.Quantity > ParcialContratos)
			{
				...
				if ((isLong && preco >= alvoParcialPrice) || (!isLong && preco <= alvoParcialPrice))
				{
					...
					pnlParcialUsd = ptsParcial * 0.50 * ParcialContratos;  // MYM = $0,50/ponto (estimativa p/ log)
		...
		private void FechaPosicao(string motivo)
		{
			...
			if (UsarSaidaParcial && parcialFeita)
			{
				...
				double pnlFinalUsd = ptsFinal * 0.50 * contratosRestantes;  // MYM = $0,50/ponto (estimativa p/ log)
		...
		private string CaminhoArquivoPnl()
		{
			return Path.Combine(
				...
				"NinjaTrader 8", "BotAprovacaoDow_MYM_pnl_diario.txt");
		}
		...
		[NinjaScriptProperty]
		[Range(1, 50)]
		[Display(Name="Contratos", Description="Qtd de MYM (5 = conservador; backtest 01/09 tem pico em 8-10)", Order=1, GroupName="1. Geral")]
		public int Contratos { get; set; }

		[NinjaScriptProperty]
		[Range(1, 500)]
		[Display(Name="Alvo (pontos)", Description="Take profit em pontos Dow (117 = grid MYM 01/09)", Order=10, GroupName="2. Saida")]
		public double AlvoPontos { get; set; }

		[NinjaScriptProperty]
		[Range(0.25, 100)]
		[Display(Name="Stop (pontos)", Description="Stop loss em pontos Dow (29 = $72,50/trade com 5 MYM; grid MYM 01/09)", Order=11, GroupName="2. Saida")]
		...
		[NinjaScriptProperty]
		[Range(0, 50)]
		[Display(Name="Tolerancia toque (ticks)", Description="Distancia max da linha p/ contar como toque. MYM tick=1pt -> 12 ticks = 12pt.", Order=15, GroupName="2. Saida")]
		public int TolToqueTicks { get; set; }

		[NinjaScriptProperty]
		[Range(0, 200)]
		[Display(Name="Max dist. entrada (pontos)", Description="Close deve estar a no max X pontos Dow da linha (0=sem filtro, 35=grid MYM 01/09). Filtra entradas chase.", Order=16, GroupName="2. Saida")]
