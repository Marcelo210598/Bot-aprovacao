...
## Última atualização: 2026-09-10 — 🔴 **ABERTURA DE NY levada pro NT8 nativo (`src/AberturaExplosao.cs`, 7 builds) — SEM EDGE.** O harness expôs que o "+$4-5/trade" de 09/09 era **look-ahead intra-segundo** (64-84% dos pregões o 1º s toca +3t E −3t; o sim adivinhava a direção vencedora pela cor do bar). Trigger causal honesto: seguir o rompimento = −$11/trade (= controle "sempre long"). No NT8 Market Replay (MNQ JUN26, tick), testadas 7 versões — seguir/fadar, stop apertado/largo, trail em tick/$, espera 0-30s: **tudo breakeven a negativo.** v6 (stop 12t): net −$18/9 dias. v7 (StopDolar $250 / TrailDolar $100 / Espera 10): net **−$830/6 dias**, toda perda bate o stop cheio, entrou LONG 3× perdeu 3. **Sizing decidido: 6 MNQ.** Descobertas técnicas: `Time[0]` no NT8 = horário da BARRA (não do tick) → lógica de tempo < 1min tem que ir pro `OnMarketData`; fuso-proof por reflection (VM em BRT, NT8 global em ET). Params novos: `EsperaSegundos` (lê o caminho da abertura e segue), `StopDolar`/`TrailDolar`/`RespiroSegundos` (trailing em $, ideia do Marcelo), `InverterDirecao`. **➡️ 11/09: Marcelo NÃO aceitou o no-go — vai mexer mais no stop/trail (Espera 5, StopDolar menor, TrailDolar 30-50). ANTES: 1 run LIMPO full-period 01→17/06 com `TradeWindow` setado (não 6 dias no olho). Se ainda negativo → cravar no-go, ir pro próximo (firma DD estático / estratégia nova).** Detalhe: `docs/abertura-ny-resultados-10-09.md` + `historico/2026-09-10.md`. Produção (`BotAprovacao.cs`) e ONFADE INTOCADOS.
