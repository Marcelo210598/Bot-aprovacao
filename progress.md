...
## Última atualização: 2026-09-14 — 🌙 **`AberturaNYSpecAndersson_v3.cs` criado (motor V2 + 2 janelas de entrada independentes, manhã 09:30 ET e noite 18:00 ET, cada uma com flatten próprio) a pedido do Marcelo: testar a abertura noturna (19h Brasília) sozinha antes de juntar com a manhã.** Default: só noite ativada. **Forward test manual no Market Replay, junho/2026 inteiro (18 pregões): FECHOU EM −$650,00, 18 trades, 5 ganhos/13 perdas (28% WR).** Quebra por motivo: STOP (8×, sempre perda cheia) = −$909,50; BE (3×, nunca protegeu) = −$72,00; TRAIL (7×, 71% WR) = +$331,50 — é a única fonte de lucro, mas não cobre o resto. **Não bateu o critério combinado com o Marcelo pra avançar direto pro combo manhã+noite.** Dois bugs achados e corrigidos no caminho: (1) `FlushFiles` sem guard de 0-barras crashava com "Index out of range" antes de rodar o Replay; (2) o código só contava o ÚLTIMO fill quando entrada/saída enchia em mais de 1 pedaço (comum no Replay) — jogava fora metade do PnL. Corrigido acumulando média ponderada de entrada e saída até a posição ficar flat; validado trade a trade contra a aba Execuções e contra a linha `[MeuTrade]` do log (bate exato em todos os dias conferidos). Log completo trade a trade: `docs/forward-test-noturna-v3-junho.md`. Pendente: rodar **manhã sozinha** e **manhã+noite juntas** (mesmo junho) antes de decidir o próximo passo — decisão em aberto com o Marcelo. Produção (`BotAprovacao.cs`) e ONFADE INTOCADOS.

## 2026-09-11 — 🟢 **`AberturaNYSpecAndersson.cs` (V1+V2): primeiro resultado do projeto que sobrevive 45 dias reais de tick replay sem estourar o DD.** Manhã: `AberturaExplosao.cs` levou o no-go definitivo (9 configs testadas, 6 dias reais 6/6 negativos, 6 ideias novas A-F sem edge, grid Python corrigido de um bug que inflava resultado). Pivot pro pedido formal do Andersson (gatilho 10t/1ª vela, BE progressivo em degraus) → **`src/AberturaNYSpecAndersson.cs`** criado (motor do AberturaExplosao, defaults do Andersson: 6 contr/$250/$500/BE 100-0-50). **Forward test 45 dias (jun+jul/2026): V1 fecha −$340,50 (PF 0,87)** — mas a curva de patrimônio CONTÍNUA (sem resetar por mês) tem DD de **$1.162,50, que ESTOURARIA o limite $1.000 EOD.** Análise de comitê (8 personas) achou o motor positivo (`AbTrail` +$84,81/trade, n=24) e o problema (`AbStop` −$198,08/trade, slippage do stop sintético em spike). Marcelo pediu reduzir risco antes de aplicar a correção: **`src/AberturaNYSpecAndersson_v2.cs`** (4 contr/$150/BE 60-0-30, arquivo separado, V1 preservado intocado). **V2 nos MESMOS 45 dias: +$408,00 (PF 1,34), DD contínuo $524 — NÃO ESTOURA.** Mecanismo confirmado (44/45 dias mesmo sinal V1×V2, só 1 exceção real explicável). Classificação: INVESTIGATE. Próximo passo recomendado: trocar stop sintético por ordem nativa do NT8. Marcelo cogitando usar como bot PÓS-aprovação (não pra aprovar conta). Amanhã: testar V3 com 3 contratos. Detalhe: `historico/2026-09-11.md` (12 seções) + `docs/analise-comite-aberturaNYSpecAndersson-11-09.md` + `docs/resumo-para-andersson-11-09.md`. Produção (`BotAprovacao.cs`) e ONFADE INTOCADOS.

---

## 2026-09-08 — ✅ **MILESTONE: Fase A do ONFADE aprovada.** `OnFadeHarness.cs` (NT8) reproduz `onfade_harness.py` (Python) **bit a bit**: 77/77 barras-chave idênticas + **38/38 trades idênticos** (entrada, stop, alvo, timestamps, motivo de saída, PnL bruto e líquido). Total líquido −US$ 134,72 nos dois lados, 6 wins/38. Único diff = `MNQ` vs `MNQZ4` (cosmético). ⚠️ Recorte curto (6 semanas) — **NÃO é resultado de rentabilidade**, só prova de equivalência lógica. Escrito `src/OnFadeNative.cs` (Fase B, execução nativa) — não compilado. Próximo: recompilar → confirmar comissão real → B0 (sanidade) → B1/B2/B3 (econômico, veredito = Strategy Analyzer). Detalhe: `historico/2026-09-08.md`.
